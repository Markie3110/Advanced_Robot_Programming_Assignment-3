...
// int read_and_echo(int read_fd, char *readbuf, char *message1, char *message2, char *message3, char *message4)
// {
//     bzero(readbuf, sizeof(readbuf));
//     int n1, n2;
//     block_signal(SIGUSR1);
//     n1 = read(read_fd, readbuf, sizeof(readbuf));
//     unblock_signal(SIGUSR1);
//     //logstring(logpath, "readbuf", readbuf);
//     if (n1 < 0)
//     {
...
//         block_signal(SIGUSR1);
//         n2 = write(read_fd, readbuf, sizeof(readbuf));
//         unblock_signal(SIGUSR1);
//         if (n2 < 0)
//         {
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if(FD_ISSET(server_socket, &cfds))
        {
            ...
            block_signal(SIGUSR1);
            n = read(new_client_socket, socket_read_buf, sizeof(socket_read_buf));
            unblock_signal(SIGUSR1);
            if (n < 0)
            {
                logerror(logpath, "Error: Socket ID read", errno);
            }
            else
            {
                logmessage(logpath, "Read ID message");

                // Echo the message
                block_signal(SIGUSR1);
                n = write(new_client_socket, &socket_read_buf, sizeof(socket_read_buf));
                unblock_signal(SIGUSR1);
                if (n < 0)
                {
                ...
                {
                    logmessage(logpath, "Echoed target ID message");
                }
            }
            // n = read_and_echo(new_client_socket, socket_read_buf, "Error: Socket ID read", "Read ID message", "Error: Target ID echo", "Echoed target ID message");
            // if (n > 0)
            // {
            ...
            block_signal(SIGUSR1);
            n = write(new_client_socket, socket_write_buf, sizeof(socket_write_buf));
            unblock_signal(SIGUSR1);
            if (n < 0)
            {
                logerror(logpath, "Error: Socket size write", errno);
            }
            else
            {
                logmessage(logpath, "Sent window size");
                // Wait for the echo from client
                bzero(socket_read_buf, sizeof(socket_read_buf));
                logint(logpath, "fd", new_client_socket);
                block_signal(SIGUSR1);
                n = read(new_client_socket, socket_read_buf, sizeof(socket_read_buf));
                unblock_signal(SIGUSR1);
                if (n < 0)
                {
                    logerror(logpath, "Error: Socket size read", errno);
                }
                else
                {
                    if (strcmp(socket_read_buf, socket_write_buf) == 0)
                    {
                        logmessage(logpath, "Read size echo");
                        logstring(logpath, "Size echo", socket_read_buf);
                    }
                    else
                    {
                        logstring(logpath, "writebuf", socket_write_buf);
                        logerror(logpath, "Error: Wrong size echo", errno);
                        logstring(logpath, "Wrong size echo", socket_read_buf);
                    }
                }
            }
        }
        ...
        for (int i=0; i < 2; i++)
        {
            ...
            if (FD_ISSET(client_socket_list[i], &cfds))
            {
                if (client_socket_list[i] == client_socket_list[0])
                {
                    ...
                    block_signal(SIGUSR1);
                    n = read(client_socket_list[i], socket_target_read_buf, sizeof(socket_target_read_buf));
                    unblock_signal(SIGUSR1);
                    if (n < 0)
                    {
                        logerror(logpath, "Error: Target pos read", errno);
                    }
                    else
                    {
                        printf("%s\n", socket_target_read_buf);
                        logmessage(logpath, "Read target pos");
                        // Echo the target positions
                        block_signal(SIGUSR1);
                        n = write(client_socket_list[i], socket_target_read_buf, sizeof(socket_target_read_buf));
                        unblock_signal(SIGUSR1);
                        if (n < 0)
                        {
                            logerror(logpath, "Error: Target pos echo", errno);
                        }
                        else
                        {
                            logmessage(logpath, "Echoed target pos");
                        }
                    }
                ...
                else if (client_socket_list[i] == client_socket_list[1])
                {
                    ...
                    block_signal(SIGUSR1);
                    n = read(client_socket_list[i], socket_obstacle_read_buf, sizeof(socket_obstacle_read_buf));
                    unblock_signal(SIGUSR1);
                    if (n < 0)
                    {
                        logerror(logpath, "Error: Obstacle pos read", errno);
                    }
                    else
                    {
                        logmessage(logpath, "Read obstacle pos");
                        // Echo the object positions
                        block_signal(SIGUSR1);
                        n = write(client_socket_list[i], socket_obstacle_read_buf, sizeof(socket_obstacle_read_buf));
                        unblock_signal(SIGUSR1);
                        if (n < 0)
                        {
                            logerror(logpath, "Error: Obstacle pos echo", errno);
                        }
                        else
                        {
                            logmessage(logpath, "Echoed obstacle pos");
                        }
                    }
            ...
            if(FD_ISSET(fd1[i], &rfds1))
            {
                ...
                else if (fd1[i] == UI2serverdone_read)
                {
                    ...
                    if (((strcmp(UIdone_buf, "GE"))==0) && (client_socket_list[0] != 0))
                    {
                        ...
                        block_signal(SIGUSR1);
                        n = write(client_socket_list[0], socket_target_write_buf, sizeof(socket_target_write_buf));
                        unblock_signal(SIGUSR1);
                        if (n < 0)
                        {
                            logerror(logpath, "Error: GE write", errno);
                        }
                        else
                        {
                            logmessage(logpath, "Sent GE message"); 
                            // Wait for an echo from the client
                            block_signal(SIGUSR1);
                            n = read(client_socket_list[0], socket_target_read_buf, sizeof(socket_target_read_buf));
                            unblock_signal(SIGUSR1);
                            if (n < 0)
                            {
                                logerror(logpath, "Error: GE read", errno);
                            }
                            else
                            {
                                if (strcmp(socket_target_read_buf, socket_target_write_buf) == 0)
                                {
                                    logmessage(logpath, "Read GE echo");
                                    logstring(logpath, "GE echo", socket_target_read_buf);
                                }
                                else
                                {
                                    logerror(logpath, "Error: Wrong echo", errno);
                                    logstring(logpath, "Wrong GE echo", socket_target_read_buf);
                                }
                            }
                        }
                    }
        ...
        if (flag == 1)
        {
            ...
            if (n < 0)
            {
                logerror(logpath, "Error: Socket STOP write", errno);
            }
            else
            {
                logmessage(logpath, "Sent STOP message");
            }


            // Wait for an echo from the target
            block_signal(SIGUSR1);
            n = read(client_socket_list[0], socket_target_read_buf, sizeof(socket_target_read_buf));
            unblock_signal(SIGUSR1);
            if (n < 0)
            {
                logerror(logpath, "Error: Target STOP echo", errno);
            }
            else
            {
                if (strcmp(socket_target_read_buf, socket_target_write_buf) == 0)
                {
                    logmessage(logpath, "Read target echo");
                    logstring(logpath, "Target echo", socket_target_read_buf);
                }
                else
                {
                    logerror(logpath, "Error: Wrong echo", errno);
                    logstring(logpath, "Wrong STOP echo", socket_target_read_buf);
                }
            }
            ...
            block_signal(SIGUSR1);
            n = write(client_socket_list[1], socket_obstacle_write_buf, sizeof(socket_obstacle_write_buf));
            unblock_signal(SIGUSR1);
            if (n < 0)
            {
                logerror(logpath, "Error: Socket STOP write", errno);
            ...
            // Wait for an echo from the obstacle
            block_signal(SIGUSR1);
            n = read(client_socket_list[1], socket_obstacle_read_buf, sizeof(socket_obstacle_read_buf));
            unblock_signal(SIGUSR1);
            if (n < 0)
            {
                logerror(logpath, "Error: Socket obstacle read", errno);
            }
            else
            {
                if (strcmp(socket_obstacle_read_buf, socket_obstacle_write_buf) == 0)
                {
                    logmessage(logpath, "Read obstacle echo");
                    logstring(logpath, "obstacle echo", socket_obstacle_read_buf);
                }
                else
                {
                    logerror(logpath, "Error: Wrong echo", errno);
                    logstring(logpath, "Wrong obstacle echo", socket_obstacle_read_buf);
                }
            }
