...
int main(int argc, char *argv[])
{
    ...
    // Declare the buffers for the pipes
    ...
    // Declare a fd_set for the file descriptors
    fd_set rfds;
    struct timeval tv;
    FD_ZERO(&rfds);
    ...
    while(1)
    {
        ...
        // Reset the buffers
        bzero(serverreadpos_buf, sizeof(serverreadpos_buf));
        bzero(serverreadtary_buf, sizeof(serverreadtary_buf));
        bzero(serverreadtarx_buf, sizeof(serverreadtarx_buf));
        bzero(serverreadobsy_buf, sizeof(serverreadobsy_buf));
        bzero(serverreadobsx_buf, sizeof(serverreadobsx_buf));
        bzero(serverwrite_buf, sizeof(serverwrite_buf));
        bzero(serverwritedone_buf, sizeof(serverwritedone_buf));
        bzero(serverack_buf, sizeof(serverack_buf));
        ...
        for (int i = 0; i < (sizeof(targets_YPos)/4); i++)
        {
            ...
            {
                logmessage(logpath, "Target fine");
            }
        }
