...
int main(int argc, char *argv[])
{
    ...
    n = write_and_echo(network_socket, socket_write_buf, socket_read_buf, sizeof(socket_write_buf), sizeof(socket_read_buf), "Error: Socket write", "Sent ID message", "Error: Target ID echo", "Read ID echo", "Error: Wrong echo");
    if (n > 0)
    {
    ...
    // Declare a variable to keep track of the number of obstacles
    ...
    while(1)
    {
        ...
        n = write_and_echo(network_socket, socket_write_buf, socket_read_buf, sizeof(socket_write_buf), sizeof(socket_read_buf), "Error: Obstacle_pos write", "Sent obstacle_pos", "Error: Obstacle_pos echo", "Read obstacle_pos echo", "Error: Wrong obstacle_pos echo");
